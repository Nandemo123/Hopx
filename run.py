...
def banner() -> None:
    clear()
    owner_field = f"{OWNER_NAME:<34}"[:34]  # truncate to keep box alignment intact
    art = f"""
{C.cyan}{C.bold}╔══════════════════════════════════════════════════════════════╗
...
def save_config(cfg: Dict[str, Any]) -> None:
    ...
    # Open with mode 0600 from creation to avoid a window where the file
    # is briefly readable with default (umask) permissions before being
    # restricted. If the file already existed with looser permissions,
    # explicitly tighten them afterward and warn if that fails.
...
def confirm_destructive(prompt: str) -> bool:
    """Ask the user to type DELETE to confirm an irreversible action."""
    confirm = input(color(prompt, C.red)).strip()
...
def create(api_key: str) -> Any:
    ...
    # Only these keywords indicate the failure is actually about the
    # timeout value itself; anything else (auth, quota, template errors...)
    # would fail identically on every retry and shouldn't be masked by
    # 11 pointless attempts with a misleading final message.
    timeout_related_hints = ("timeout", "duration", "ttl", "exceed", "too large", "too long")
    ...
    for timeout in timeout_tries:
        try:
            ...
            last_error = e
            msg = str(e).lower()
            if not any(hint in msg for hint in timeout_related_hints):
                bad(f"sandbox creation failed (not a timeout issue): {e}")
...
def action(api_key: str, name: str) -> None:
    ...
    if name == "start":
        ...
        if not confirm_destructive(f"Delete {sid} permanently? type DELETE: "):
            warn("cancelled")
            return
        sb.kill()
        ok("deleted")
...
def preview_url(sb: Any, port: int) -> str:
    ...
    try:
        ...
        # The port-prefixed subdomain scheme assumes a "sub.domain.tld"
        # style host; without at least one dot, prefixing a port would
        # produce a malformed URL rather than a useful preview link.
        if "." not in rest:
            return ""
    ...
    while True:
        ...
        try:
            ...
            if cmd.startswith("download "):
                ...
                # sb.files.read() may return bytes for binary files. Writing
                # str(data) in that case would corrupt the file by saving
                # its Python repr (e.g. "b'\\x89PNG...'") instead of the
                # actual content.
            ...
            if cmd.startswith("preview"):
                ...
                continue
            if cmd == "delete":
                if confirm_destructive("Delete sandbox permanently? type DELETE: "):
                    sb.kill()
                    ok("deleted")
                    return
                warn("cancelled")
...
def menu(api_key: str) -> None:
    while True:
        ...
        try:
            ...
            # ResourceLimitError/APIError may both be aliased to the base
            # Exception class if hopx_ai.exceptions failed to import; in
            # that case, separate except clauses for them would collapse
            # into the first one and mislabel unrelated errors. Checking
            # explicitly (and only trusting the label when the SDK really
            # provided distinct classes) keeps the message accurate.
            if ResourceLimitError is not Exception and isinstance(e, ResourceLimitError):
                bad(f"Resource limit: {e}")
            elif APIError is not Exception and isinstance(e, APIError):
                bad(f"API error: {e}")
            else:
                bad(str(e))
            pause()
